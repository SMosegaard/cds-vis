...
You can download the dataset [here](https://www.kaggle.com/datasets/patrickaudriaz/tobacco3482jpg?resource=download) and place it in the in folder. Make sure to unzip the data within the folder before executing the code.
...
Additionally, the choice of optimizer plays a crucial role in model training. The models were tested with both SGD and adam optimizers to assess their performance under different optimization strategies. Generally, the adam optimizer performs very well on complex data such as images and is robust to noisy data, however it does cause some overfitting. Adam is an  adaptive learning rate algorithm, which leads to rapid convergence but also an increased risk of the model fitting too closely to the training data and failing to generalize to unseen data.

It is also worth noting that the models were trained for a relatively small batch size (32) and number of epochs (10 epochs) due to computational constraints. While the models may not have fully converged within this setup, the results still provide valuable insights into their performance and capabilities. It would be relevant to further explore the potential of the models with longer training.
...
<img src = "https://raw.githubusercontent.com/SMosegaard/cds-vis/main/assignments/assignment-3/out/heatmap_Scientific10064589_10064594.jpg.png" width = "800">
