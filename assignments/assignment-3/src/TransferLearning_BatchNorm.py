...
def define_model():
    ...
    Subsequently, a new fully connected layer with ReLU activation is added followed by batch normalization.
    Finally,  an output layer with softmax activation is added for multi-class classification.
    """
...
def evaluate(X_test, y_test, model, H, optimizer):
    ...
    filepath_metrics = open(f'out/VGG16_metrics_BatchNorm_{optimizer}.txt', 'w')
    filepath_metrics.write(classifier_metrics)
    filepath_metrics.close()

    plot_history(H, 10, f"out/VGG16_losscurve_BatchNorm_{optimizer}.png")
